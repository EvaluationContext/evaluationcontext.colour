---
title: Home
---

# EvaluationContext.Colour

A DAX UDF library for manipulation of hex colours for Power BI

[Download From DaxLib :material-download:{ style="color: var(--button-color); font-size: 1.2em;" }](https://daxlib.org/package/EvaluationContext.Colour/){ .md-button }
[Download Example PBIP :material-download:{ style="color: var(--button-color); font-size: 1.2em;" }](https://github.com/EvaluationContext/EvaluationContext.Colour/tree/main/PowerBI){ .md-button } 

!!! tip "Getting Started"

    1. Download the library from [DaxLib](https://daxlib.org/package/EvaluationContext.Colour/)
    2. Install using TMDL view
    3. Start using the functions in your measures
     
    For detailed examples, check out our example [PBIP file](https://github.com/EvaluationContext/EvaluationContext.Colour/tree/main/PowerBI)

![Library](./assets/images/UDFInAction.png)